# SDM_V1 - Strategy Decision Model
...
Purpose: Verify statistical edge, prevent overfitting, and ensure signal integrity before deployment into production or recommendation generation.
Inputs:
...
- Validation pipeline approvals/rejections
Dependencies:
- SDM-04 Signal Discovery
Failure Conditions:
...
- Require statistical significance tests (e.g., t-stat, Deflated Sharpe) as deployment gates; standard confidence intervals are insufficient.
Outputs:
- Confidence rating
...
- [VALIDATION_REQUIRED] Framework for multi-broker aggregate margin exposure limits is unresolved.
Human Visibility Requirements:
...
Outputs: Position count directives, drawdown halt triggers, concentration limit enforcement directives.
...
Dependencies: SDM-11 Position Management, SDM-16 Signal Lifecycle.
...
---
[SDM-13 Attribution]
Purpose: [LOW CONFIDENCE] Evaluate the effectiveness of past decisions and accurately assign performance impact to specific signals, models, or human overrides.
Inputs: [LOW CONFIDENCE]
Decision Logic:
- [LOW CONFIDENCE] Missing mathematical framework for isolating and attributing performance edge to specific variables [VALIDATION_REQUIRED].
Outputs: [LOW CONFIDENCE]
Dependencies: [LOW CONFIDENCE]
Failure Conditions: [LOW CONFIDENCE]
Human Visibility Requirements: [LOW CONFIDENCE]
Audit Requirements: [LOW CONFIDENCE]
---

---
[SDM-14 Research Intake]
Purpose: [LOW CONFIDENCE] Determine the relevance, credibility, and initial categorization of incoming unstructured market information and fundamental research.
Inputs: [LOW CONFIDENCE]
Decision Logic:
- [LOW CONFIDENCE] Missing heuristic or mathematical framework for scoring and filtering raw research documents before signal discovery [VALIDATION_REQUIRED].
Outputs: [LOW CONFIDENCE]
Dependencies: [LOW CONFIDENCE]
Failure Conditions: [LOW CONFIDENCE]
Human Visibility Requirements: [LOW CONFIDENCE]
...
Inputs: Market Regime Classification, AI Sentiment Uncertainty Quantile Bands, System Telemetry, Margin Limits.
Decision Logic:
...
Outputs: Hard recommendation halts (Kill-Switches), Dynamic margin scaling adjustments, Human approval gates, Critical alert states.
Dependencies: SDM-03 Market Regime Classification, SDM-07 Expected Value Assessment, SDM-11 Position Management.
Failure Conditions: Portfolio drawdown exceeds 5%; Unapproved trade attempted; Uncertainty bands widen beyond threshold; System data latency exceeds safe limits.
...
Audit Requirements: Log every human approval or rejection; log all drawdown limit tests; log all triggered halts, latency threshold breaches, and scaling adjustments.
---

---
[SDM-16 Signal Lifecycle]
Purpose: Manage the duration, holding period, and exit conditions of generated signals to enforce risk controls and maximize probability of returns.
...
- Automatically reduce execution or recommendation sizing triggered by widening uncertainty bands (e.g., quantile bands).
- Apply volume-based execution limits relative to trailing volume.
...
Outputs: Exit suggestions, Risk summaries, Position limit halts, Bid size reduction triggers.
Dependencies:
- [VALIDATION_REQUIRED] Exact mathematical indicators to prove an equity has exited an ergodic state and entered an unpredictable tail-risk regime.
Failure Conditions: Execution size cannot scale down during widening uncertainty bands. Macro shock flags trigger non-ergodic breakdowns.
Human Visibility Requirements: Clear presentation of exit suggestions, rationale for extending holding periods, and risk summaries.
Audit Requirements: Log all decisions to extend trade validity beyond expected horizons. Log triggers for position limit halts and uncertainty band widening.
---
