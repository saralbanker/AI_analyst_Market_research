...
**Problem:** 
Without a mechanism to isolate skill (alpha) from market luck (beta), the system cannot identify decaying edges, threatening 5-year survivability.

**Decision:** 
Establish a tag-based expectancy matrix tracking Profit Factor against qualitative setup parameters.

**Constitutional Rule:** 
The SDM shall attribute and evaluate historical edge exclusively by segmenting trade expectancy and profit factors across mandatory qualitative metadata tags: Setup Type, Market Regime Context, and Holding Duration.

**Rationale:** 
Provides a minimum viable learning loop that tracks exactly which specific setups succeed in specific environments, avoiding the need for computationally massive institutional factor models.
...
**Validation Requirements:** 
All signal generations and executed trades must permanently inherit their generating metadata tags, and the system must maintain a closed loop linking closed P&L back to these tags.
